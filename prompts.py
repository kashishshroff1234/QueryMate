...
COLUMN_GLOSSARY = """
- CRITICAL: "revenue", "total_quantity", "sales", "total sold", and similar terms are NEVER actual
  column names anywhere in this schema. They do not exist as columns. If you write
  "SELECT revenue ..." or "SELECT total_quantity ..." as if these were real columns, the query
  will be rejected. You must always compute them as expressions.
- "revenue" means quantity * unit_price, computed by joining orders to products on product_id.
  Always write it as an expression, e.g. SUM(o.quantity * p.unit_price) AS revenue. Unless the
  question says otherwise, compute revenue only over orders.status = 'completed'.
- "total sold" or "units sold" means SUM(orders.quantity), written as an expression and aliased,
  e.g. SUM(o.quantity) AS total_quantity. It is never a pre-existing column.
- "most sold product" requires grouping by product and ordering by SUM(quantity) DESC, not by
  selecting a quantity column directly.
...
FEW_SHOTS = [
    ...
    {
        "q": "What's the most sold product by quantity?",
        "sql": (
            "SELECT p.name, SUM(o.quantity) AS total_quantity "
            "FROM orders o JOIN products p ON o.product_id = p.product_id "
            "WHERE o.status = 'completed' "
            "GROUP BY p.name ORDER BY total_quantity DESC LIMIT 1"
        ),
    },
    {
        "q": "Which products in the Sports category drive the most revenue?",
        "sql": (
            "SELECT p.name, ROUND(SUM(o.quantity * p.unit_price), 2) AS revenue "
            "FROM orders o JOIN products p ON o.product_id = p.product_id "
            "WHERE p.category = 'Sports' AND o.status = 'completed' "
